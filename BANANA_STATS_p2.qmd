...
```{r}
# Ensure your time series data is properly formatted as a ts object
# Assuming banana_tsibble is the dataset
...
- The trend shows a gradual increase in banana prices from the 1960s to the mid-2000s, followed by a sharp rise in the late 2000s and a subsequent decline after 2010. This long-term component reflects significant upward movement over time with a peak before 2010.
- The seasonal component exhibits a consistent repeating pattern throughout the entire period, with slight fluctuations around the mean. The amplitude of these seasonal cycles remains stable, indicating regular periodic variations in banana prices, likely linked to annual production or consumption cycles.
...
ggAcf(banana_ts_transformed, main = "ACF of Transformed Data")
ggPacf(banana_ts_transformed, main = "PACF of Transformed Data")

# Perform Augmented Dickey-Fuller Test
adf_test <- adf.test(banana_ts_transformed, alternative = "stationary")
...
ggAcf(banana_ts_diff, main = "ACF of Differenced Data")
ggPacf(banana_ts_diff, main = "PACF of Differenced Data")

# Re-run ADF Test on Differenced Data
adf_test_diff <- adf.test(banana_ts_diff, alternative = "stationary")
...
arima_model_1 <- Arima(banana_ts_transformed, order = c(1, 1, 1), seasonal = c(1, 1, 0))

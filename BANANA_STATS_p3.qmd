...
arma_models <- banana_returns |>
  model(
    ARMA_1_1 = ARIMA(Return ~ pdq(1,0,1)),
    ARMA_2_1 = ARIMA(Return ~ pdq(2,0,1)),
    ARMA_1_2 = ARIMA(Return ~ pdq(1,0,2)),
    ARMA_2_2 = ARIMA(Return ~ pdq(2,0,2))
  )
...
as the p-value is below 0,05 it indicates that ARCH effect is present. Heteroscedasticity is present and therefore we have to proceed with fitting GARCH model

DMYTRO AND OTHERS PLEASE DOBLE CHECK RESULTS!
...
PLEASE FIND HOW THE GROUP NUMBER (20,30,40,50 etc) AFFECT THE MODEL FIT
## Forecast ARMA model

...
ARMA(1,1): Uses one past value (AR(1)) and one past shock (MA(1)). It accounts for short-term dependencies in both price levels and shocks.
ARMA(2,1): Expands the autoregressive component to two past values (AR(2)), capturing longer memory effects in price behavior, while still using one past shock.
ARMA(1,2): Keeps the autoregressive component small (AR(1)) but allows for two past shocks (MA(2)), which can help account for more complex error structures.
ARMA(2,2): Uses two past values and two past shocks, making it a more flexible model that can capture both medium-term dependencies and more intricate noise patterns.
Model Comparison:
...
plot(volatility_forecast, type = "l", col = "blue",
     main = "GARCH(3,1) Volatility Forecast",
     xlab = "Time", ylab = "Volatility")

plot(return_forecast, type = "l", col = "red",
     main = "GARCH(3,1) Return Forecast",
